...
description: Automatically analyzes CSV files with comprehensive statistical summaries, data quality checks, and intelligent visualizations. Detects data types (sales, customer, financial, operational, survey) and adapts analysis accordingly. Generates correlation heatmaps, time-series plots, distribution charts, and categorical breakdowns. Use when user uploads CSV, requests data analysis, asks for insights, or wants to understand dataset structure. NO user prompting required - immediately runs full analysis with all relevant visualizations based on detected data patterns. Supports missing data analysis, automatic date column detection, and industry-specific insights. Built with pandas, matplotlib, and seaborn.
---
...
GLM should use this Skill whenever the user:
